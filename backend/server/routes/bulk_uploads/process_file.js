    ...
    handler: async (request, h) => {
      try {
        if (request.payload.Message) {
          ...
          const authToken = request.payload.authToken
          ...
          await setJsonData(redis, elasticacheKey, {
            stage: 'Scanning upload',
            status: 'working'
          })
          const scanResult = await scanResults(
          ...
          if (scanResult?.data?.scanComplete === true) {
            if (scanResult?.data?.scanResult === 'THREATS_FOUND') {
              ...
              if (response.errorMessage) {
                ...
                await setJsonData(redis, elasticacheKey, {
                  stage:
                    response.data?.invalid?.length +
                    response.data?.valid?.length +
                    ' locations validated',
                  status: 'working'
                })
                // Check invalid locations for duplicates
                const locationNames = await listLocationNames(
                  redis,
                  sessionData?.orgId
                )
                ...
                for (let i = 0; i < validLength; i += 25) {
                  showProgress &&
                    setJsonData(redis, elasticacheKey, {
                      stage: `Associating flood data (${Math.round(
                        (i / validLength) * 100
                      )}%)`,
                      status: 'working'
                    })
                ...
                await setJsonData(redis, elasticacheKey, {
                  ...
                  status: 'rejected',
                  error: [
                    { errorType: 'generic', errorMessage: 'Unknown error' }
                  ]
                })
              }
            } else {
              await setJsonData(redis, elasticacheKey, {
                ...
                status: 'rejected',
                error: [
                  { errorType: 'generic', errorMessage: 'Error Scanning File' }
                ]
              })
            }
          } else {
