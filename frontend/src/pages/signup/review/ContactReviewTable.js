...
export default function ContactReviewTable ({ profile }) {
  ...
  const ShowMobileNumber = () => {
    return (
      <>
      {profile.mobilePhones.length > 0 || profile.unverified.mobilePhones.length > 0 ? (
          <tbody className='govuk-table__body'>
  ...
  const ShowHomePhone = () => {
    return (
      <>
      {profile.homePhones.length > 0 || profile.unverified.homePhones.length > 0 ? (
          <tbody className='govuk-table__body'>
            ...
            {profile.unverified.homePhones.map((unregisteredHomePhones, index) => (
              <tr key={index} className='govuk-table__row'>
              <td className='govuk-table__header govuk-!-width-one-half'>
                By phone call
              </td>
