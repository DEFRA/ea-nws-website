...
#Not working because cannot get the API/Backend to run, need to checkmore tomorrow
def test_SignInStart_validEmail(get_browser):
    browser = get_browser
    browser.get(url)
    browser.find_element(By.ID, "emailAddress").send_keys("valid@email.uk")
    browser.find_element(By.CLASS_NAME, "govuk-button").click()
    assert browser.current_url("http://localhost:3000/CheckYourEmailPage")
    browser.close()

def test_SignInStart_invalidEmail(get_browser):
    browser = get_browser
    browser.get(url)
    browser.find_element(By.ID, "emailAddress").send_keys("invalid@email.uk")
    browser.find_element(By.CLASS_NAME, "govuk-button").click()
