...
different_landline_url = "http://localhost:3000/signup/contactpreferences/landline/choosenumber"
...
def setup_validate_only_landline(get_browser):
    browser = get_browser
    browser.get(index)
    button_xpath = f"//button[text()='Activate/Deactivate Mock Session 1']"
    mock_session_link = browser.find_element(By.XPATH, button_xpath)
...
def test_addlandlinevalidate_enteradifferentnumber_when_number_added(get_browser):
    browser = setup_validatelandline_test(get_browser)  
    browser.find_element(By.LINK_TEXT, "Enter a different telephone number").click()
    time.sleep(1)
    assert browser.current_url == different_landline_url

def test_addlandlinevalidate_enteradifferentnumber(get_browser):
    browser = setup_validate_only_landline(get_browser)  
    browser.find_element(By.LINK_TEXT, "Enter a different telephone number").click()
    time.sleep(1)
    assert browser.current_url == previous_page
