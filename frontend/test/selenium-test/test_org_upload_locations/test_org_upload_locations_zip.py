...
infoPage = url_org_man_loc.get("add").get("shapefileInfo")
mainPage = url_org_man_loc.get("add").get("shapefileUpload")
nextPage = url_org_man_loc.get("add").get("loading")

currentDir = os.path.dirname(__file__)
validFilePath = os.path.join(currentDir, "valid.zip")
...
def test_page_flow(get_browser):
    ...
    select_input_radio_option(browser, "BulkShapefile")
    click_button(browser, "Continue", infoPage)
    assert "How to upload a location as a shapefile in a ZIP" in browser.page_source
